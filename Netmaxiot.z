...
# NetmaxIOT 4 Digit Display commands
# Initialise
...
# NetmaxIOT Chainable RGB LED commands
...
# Read temp in Celsius from NetmaxIOT Temperature Sensor
...
# Read value from NetmaxIOT Ultrasonic
...
# Read NetmaxIOT Accelerometer (+/- 1.5g) XYZ value
def acc_xyz():
...
# Read and return temperature and humidity from NetmaxIOT DHT Pro
...
# NetmaxIOT LED Bar - get current state
...
# NetmaxIOT 4 Digit Display - initialise
def fourDigit_init(pin):
	write_i2c_block(address, fourDigitInit_cmd + [pin, unused, unused])
	return 1

# NetmaxIOT 4 Digit Display - set numeric value with or without leading zeros
...
# NetmaxIOT 4 Digit Display - set brightness
# brightness: (0-7)
...
# NetmaxIOT 4 Digit Display - set individual segment (0-9,A-F)
# segment: (0-3)
# value: (0-15) or (0-F)
...
# NetmaxIOT 4 Digit Display - set 7 individual leds of a segment
# segment: (0-3)
...
# NetmaxIOT 4 Digit Display - set left and right values (0-99), with leading zeros and a colon
# left: (0-255) or (0-FF)
# right: (0-255) or (0-FF)
# colon will be lit
...
# NetmaxIOT 4 Digit Display - display analogRead value for n seconds, 4 samples per second
...
# NetmaxIOT Chainable RGB LED - initialise
...
# NetmaxIOT Chainable RGB LED - initialise and test with a simple color
# numLeds: how many leds do you have in the chain
...
# NetmaxIOT Chainable RGB LED - set one or more leds to the stored color by pattern
# pattern: (0-3) 0 = this led only, 1 all leds except this led, 2 this led and all leds inwards, 3 this led and all leds outwards
# whichLed: index of led you wish to set counting outwards from the NetmaxIOTPi, 0 = led closest to the NetmaxIOTPi
...
# NetmaxIOT Chainable RGB LED - set one or more leds to the stored color by modulo
# offset: index of led you wish to start at, 0 = led closest to the NetmaxIOT, counting outwards
...
# NetmaxIOT Chainable RGB LED - sets leds similar to a bar graph, reversible
# level: (0-10) the number of leds you wish to set to the stored color
# reversible (0-1) when 0 counting outwards from NetmaxIOT, 0 = led closest to the NetmaxIOT, otherwise counting inwards
def chainableRgbLed_setLevel(pin, level, reverse):
	write_i2c_block(address, chainableRgbLedSetLevel_cmd + [pin, level, reverse])
	time.sleep(.05)
	return 1

# NetmaxIOT - Infrared Receiver- get the commands received from the NetmaxIOT IR sensor
def ir_read_signal():
	try:
...
# NetmaxIOT - Infrared Receiver- set the pin on which the NetmaxIOT IR sensor is connected
def ir_recv_pin(pin):
